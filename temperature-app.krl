...
Show the temperature from the PDS. 
  ...
  rule appTemplate_Created {
    ...
    pre {
      ...
      mode = temperature_data{'curr_mode'}.capitalize();
      target = temperature_data{'curr_target'};
...
<span style="font-size: 96px">#{show_temp.sprintf("%.1f")}&deg;</span><span style="font-size: 88px">#{unit}</span> 
<img style="position:absolute; top:50%;" src="https://s3.amazonaws.com/kre/#{trend_img}"/><br/>
<span style="margin-top: 15px">Mode: #{mode}; Target temperature: #{target}&deg;</span><br/>
<span style="font-size:10px;float:left">Recorded: #{time:strftime(last_temp_time, '%c %Z')} </span>
</div>
